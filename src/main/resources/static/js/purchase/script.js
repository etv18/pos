...
const tblPurchaseLines = document.getElementById("tblPurchaseLines");
const btnSearchProduct = document.getElementById("btnSearchProduct");
const btnCreatePurchase = document.getElementById("btnCreatePurchase");
const pPurchaseTotal = document.getElementById("pPurchaseTotal");
const productEndpoint = "/api/v1/products";
const purchaseEndpoint = "/api/v1/purchases";
...
function calculatePurchaseLineTotal(quantity, price){
    if(
        quantity === '' ||
        quantity < 1 ||
...
function calculatePurchaseTotal(linesTotal){
    let total = Number();
    linesTotal.forEach(
        line => {
...
function setPurchaseTotalValue(table, cellIndex){
    const purchaseLineTotalCells = getColumnValuesFromTable(table, cellIndex);
    pPurchaseTotal.textContent = calculatePurchaseTotal(purchaseLineTotalCells);
}
...
function updateQuantityOnMatch(table, rowIndex, newQuantity){
    ...
    //Update price cell
    table.rows[rowIndex].cells[4].textContent = calculatePurchaseLineTotal(Number(price), Number(inputQuantity.value));
}
...
function updateQuantityOnUserInput(event){
    ...
    row.cells[4].textContent = calculatePurchaseLineTotal(Number(inputQuantity.value), Number(price));

}

function addPurchaseLineToTable(table, product){
    const tbody = table.tBodies[0];
...
function createPurchaseRequest(table){
    const tbody = table.tBodies[0];
    const rows = Array.from(tbody.rows);
    if(rows.length < 1){
        alert('Purchase must have at least one product in its body.');
        return;
    }
...
btnSearchProduct.addEventListener('click', async e => {
    ...
    if(!product) return;
    const rowIndex = checkIfProductWasInserted(product.code, tblPurchaseLines);
    if(rowIndex >= 0) {
        updateQuantityOnMatch(tblPurchaseLines, rowIndex, 1);
        setPurchaseTotalValue(tblPurchaseLines, 4);
        return;
    }
    addPurchaseLineToTable(tblPurchaseLines, product);
    setPurchaseTotalValue(tblPurchaseLines, 4);
});

btnCreatePurchase.addEventListener('click', async e => {
